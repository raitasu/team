...
import { validateLocalization } from './validateLocalization.mjs';
...
(async function lint() {
  ...
  try {
    if (shouldSkipLocalization) {
      console.log(
        chalk.yellow(`${icons.warning} Skipping localization check\n`)
      );
    } else {
      console.log(chalk.blue(`${icons.bullet} Checking localization files...`));
      await validateLocalization();
      console.log(
        chalk.green(`${icons.check} Localization files check passes\n`)
      );
    }
  } catch (err) {
    success = false;
